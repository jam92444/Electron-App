...
const PRESETS = [
  ...
  {
    label:"Last Month",
    start:(() => { const d=new Date(); d.setMonth(d.getMonth()-1,1); return d.toISOString().slice(0,10); })(),
...
const StatCard = ({ label, value, sub, color="#6366f1", icon }) => (
  <div style={{ background:"white", borderRadius:16, padding:"20px 24px", border:"1px solid #f1f5f9", boxShadow:"0 1px 4px rgba(0,0,0,.06)", display:"flex", flexDirection:"column", gap:8 }}>
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
      <span style={{ fontSize:12, fontWeight:600, color:"#94a3b8", letterSpacing:"0.06em", textTransform:"uppercase" }}>{label}</span>
      <span style={{ fontSize:20 }}>{icon}</span>
    </div>
    <div style={{ fontSize:26, fontWeight:800, color:"#0f172a", lineHeight:1.1 }}>{value}</div>
    {sub && <div style={{ fontSize:12, color:"#64748b" }}>{sub}</div>}
    <div style={{ height:3, borderRadius:9, background:color, marginTop:4, opacity:0.7 }} />
  </div>
);
...
const ChartTip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background:"#1e293b", borderRadius:10, padding:"10px 14px", fontSize:12, color:"white", boxShadow:"0 8px 24px rgba(0,0,0,.3)" }}>
      <div style={{ fontWeight:700, marginBottom:4, color:"#cbd5e1" }}>{label}</div>
      {payload.map((p,i) => <div key={i} style={{ color:p.color||"#fff" }}>{p.name}: {fmtShort(p.value)}</div>)}
    </div>
  );
};
...
const Report = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [salesRes, expRes, vendRes, monthRes, varRes, dashRes, plRes] = await Promise.all([
        window.api.getSalesDashboard(startDate, endDate).catch(e => { console.error("sales",e); return null; }),
        window.api.getExpensesByDateRange(range).catch(e => { console.error("exp",e); return { data:[] }; }),
        window.api.getTopVendors({ startDate, endDate, limit:8 }).catch(e => { console.error("vendors",e); return { data:[] }; }),
        window.api.getMonthlyPurchaseSummary({ startDate, endDate }).catch(e => { console.error("monthly",e); return { data:[] }; }),
        window.api.getVariantStockSummary().catch(e => { console.error("variant",e); return { data:[] }; }),
        window.api.getDashboardData({ startDate, endDate }).catch(e => { console.error("dash",e); return null; }),
        window.api.getProfitLoss({ startDate, endDate }).catch(e => { console.error("profitloss",e); return null; }),
      ]);
      ...
      if (salesRes?.success) {
        ...
        setSalesTotals(null); setPaymentModes([]); setTopSellingItems([]);
      }
      ...
      if (dashRes?.success) {
        ...
        setDashSummary(null); setPurchaseTrend([]);
      }
  ...
  const totalCOGS      = profitLossData?.grossProfit !== undefined 
    ? profitLossData.totalSales - profitLossData.grossProfit 
    : (dashSummary?.totalPurchaseAmount ?? 0);
  const totalExpenses  = profitLossData?.totalExpenses ?? expenses.reduce((s,e) => s+(e.amount??0), 0);
  ...
  // ── derived chart data ────────────────────────────────────────────────────
  const trendData = purchaseTrend.map(d => ({ date:d.date?.slice(5)??d.date, Purchases:d.total??0 }));

  const expensePieData = Object.entries(
    expenses.reduce((acc,e) => { const k=e.category_name??e.category??"Other"; acc[k]=(acc[k]??0)+(e.amount??0); return acc; }, {})
  ).map(([name,value]) => ({ name,value }));

  const paymentPieData = paymentModes.map(p => ({ name:p.payment_mode??"Unknown", value:p.count??0 }));

  const sizePerf = Object.entries(
    variantStock.reduce((acc,v) => { acc[v.size]=(acc[v.size]??0)+(v.quantity??0); return acc; }, {})
  ).map(([size,qty]) => ({ size,qty })).sort((a,b) => b.qty-a.qty).slice(0,10);
  ...
  const handlePrint = () => {
    ...
    const kpi = `<div class="grid">${[
      ["Revenue",fmt(totalRevenue)],["COGS",fmt(totalCOGS)],
      ["Gross Profit",fmt(grossProfit)],["Net Profit",fmt(netProfit)],
      ["Expenses",fmt(totalExpenses)],["Discounts",fmt(totalDiscount)],
      ["Bills",totalBills],["Pieces Sold",totalPieces],
    ].map(([l,v])=>`<div class="card"><div class="lbl">${l}</div><div class="val">${v}</div></div>`).join("")}</div>`;
    ...
    const vendors = topVendors.length ? `<div class="sec">Top Vendors</div><table>
    ...
    const items = topSellingItems.length ? `<div class="sec">Top Selling Items</div><table>
    ...
    const exp = expenses.length ? `<div class="sec">Expense Details</div><table>
      <tr><th>Date</th><th>Category</th><th>Description</th><th>Amount</th></tr>
      ${expenses.slice(0,30).map(e=>`<tr><td>${e.expense_date??""}</td><td>${e.category_name??""}</td><td>${e.description??""}</td><td>${fmt(e.amount)}</td></tr>`).join("")}
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>P&L Report</title>${style}</head><body>
  ...
  const applyPreset = (p) => { setActivePreset(p.label); setStartDate(p.start); setEndDate(p.end); };
    ...
    <>
      ...
      <div className="report-root">
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24, flexWrap:"wrap", gap:12 }}>
          <div>
            <h1 style={{ margin:0, fontSize:24, fontWeight:800, color:"#0f172a" }}>Profit & Loss Report</h1>
            <p style={{ margin:"4px 0 0", color:"#94a3b8", fontSize:13 }}>{startDate} → {endDate} · Generated {new Date().toLocaleDateString("en-IN")}</p>
          </div>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            <button className="btn-outline" onClick={loadData}>🔄 Refresh</button>
            <button className="btn-primary" onClick={handlePrint}>🖨️ Print / PDF</button>
          </div>
        </div>

        {/* Date Filter */}
        <div style={{ background:"white", borderRadius:14, padding:"16px 20px", border:"1px solid #f1f5f9", marginBottom:24, display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
          <span style={{ fontSize:13, fontWeight:700, color:"#374151" }}>Date Range</span>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {PRESETS.map(p => (
              <button key={p.label} className={`btn-preset ${activePreset===p.label?"active":""}`} onClick={() => applyPreset(p)}>{p.label}</button>
            ))}
          </div>
        ...
        <div ref={printRef}>
          ...
          {loading ? (
            <div className="grid-4" style={{ marginBottom:24 }}>
              {[...Array(8)].map((_,i) => <div key={i} className="skeleton" style={{ height:110 }} />)}
            </div>
          ) : (<>
            <div className="grid-4" style={{ marginBottom:16 }}>
              <StatCard label="Total Revenue"   value={fmtShort(totalRevenue)}  sub={`${totalBills} bills · ${totalPieces} pcs`} color="#6366f1" icon="💰" />
              <StatCard label="Total COGS"      value={fmtShort(totalCOGS)}     sub={`${totalPurchases} purchase orders`}        color="#f59e0b" icon="📦" />
              <StatCard label="Gross Profit"    value={fmtShort(grossProfit)}   sub={`Margin: ${grossMargin.toFixed(1)}%`}       color={grossProfit>=0?"#10b981":"#f43f5e"} icon="📈" />
              <StatCard label="Total Expenses"  value={fmtShort(totalExpenses)} sub={`${expenses.length} entries`}              color="#f43f5e" icon="💸" />
            </div>
            <div className="grid-4" style={{ marginBottom:24 }}>
              <StatCard label="Net Profit"      value={fmtShort(netProfit)}     sub={`Net margin: ${netMargin.toFixed(1)}%`}    color={netProfit>=0?"#10b981":"#f43f5e"} icon={netProfit>=0?"🟢":"🔴"} />
              <StatCard label="Avg Order Value" value={fmtShort(avgOrderValue)} sub="Per bill"                                  color="#22d3ee" icon="🧾" />
              <StatCard label="Total Discount"  value={fmtShort(totalDiscount)} sub="Discounts given"                           color="#a78bfa" icon="🏷️" />
              <StatCard label="Total Items"     value={totalItems}              sub="In catalogue"                              color="#fb923c" icon="📋" />
            </div>
          </>)}

          {/* P&L Statement */}
          <div className="chart-card" style={{ marginBottom:24 }}>
            <SectionHeader title="Profit & Loss Statement" sub="Summary financial overview for selected period" />
            <table className="pl-table">
              <thead><tr><th>Item</th><th style={{ textAlign:"right" }}>Amount</th><th style={{ textAlign:"right" }}>% of Revenue</th></tr></thead>
              <tbody>
                <tr>
                  <td style={{ fontWeight:600 }}>💰 Total Revenue</td>
                  <td style={{ textAlign:"right", fontWeight:700 }}>{fmt(totalRevenue)}</td>
                  <td style={{ textAlign:"right" }}><span className="tag tag-blue">100%</span></td>
                </tr>
                <tr>
                  <td style={{ color:"#64748b", paddingLeft:28 }}>Cost of Goods Sold (COGS)</td>
                  <td style={{ textAlign:"right", color:"#f59e0b", fontWeight:600 }}>− {fmt(totalCOGS)}</td>
                  <td style={{ textAlign:"right" }}>{pct(totalCOGS, totalRevenue)}</td>
                </tr>
                <tr className="total-row">
                  <td>📊 Gross Profit</td>
                  <td style={{ textAlign:"right", color:grossProfit>=0?"#10b981":"#f43f5e" }}>{fmt(grossProfit)}</td>
                  <td style={{ textAlign:"right" }}><span className={`tag ${grossProfit>=0?"tag-green":"tag-red"}`}>{grossMargin.toFixed(1)}%</span></td>
                </tr>
                <tr>
                  <td style={{ color:"#64748b", paddingLeft:28 }}>Discounts Given</td>
                  <td style={{ textAlign:"right", color:"#a78bfa", fontWeight:600 }}>− {fmt(totalDiscount)}</td>
                  <td style={{ textAlign:"right" }}>{pct(totalDiscount, totalRevenue)}</td>
                </tr>
                <tr>
                  <td style={{ color:"#64748b", paddingLeft:28 }}>Operating Expenses</td>
                  <td style={{ textAlign:"right", color:"#f43f5e", fontWeight:600 }}>− {fmt(totalExpenses)}</td>
                  <td style={{ textAlign:"right" }}>{pct(totalExpenses, totalRevenue)}</td>
                </tr>
                <tr className="total-row">
                  <td style={{ fontSize:15 }}>🏆 Net Profit</td>
                  <td style={{ textAlign:"right", fontSize:15, color:netProfit>=0?"#10b981":"#f43f5e" }}>{fmt(netProfit)}</td>
                  <td style={{ textAlign:"right" }}><span className={`tag ${netProfit>=0?"tag-green":"tag-red"}`}>{netMargin.toFixed(1)}%</span></td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="grid-2" style={{ marginBottom:24 }}>
            <div className="chart-card">
              <SectionHeader title="Purchase Trend" sub="Daily purchase amounts" />
              {loading ? <div className="skeleton" style={{ height:220 }} /> : (
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={trendData} margin={{ top:4, right:4, left:0, bottom:0 }}>
                    ...
                    <Area type="monotone" dataKey="Purchases" stroke="#6366f1" fill="url(#purpGrad)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
            <div className="chart-card">
              <SectionHeader title="Monthly Purchase Summary" sub="Amount & order count by month" />
              {loading ? <div className="skeleton" style={{ height:220 }} /> : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={monthlyPurchases} margin={{ top:4, right:4, left:0, bottom:0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                    <XAxis dataKey="month" tick={{ fontSize:10, fill:"#94a3b8" }} />
                    <YAxis tick={{ fontSize:10, fill:"#94a3b8" }} tickFormatter={fmtShort} />
                    <Tooltip content={<ChartTip />} />
                    <Bar dataKey="totalAmount" name="Amount" fill="#6366f1" radius={[4,4,0,0]} />
                    <Bar dataKey="purchaseCount" name="Orders" fill="#22d3ee" radius={[4,4,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
              ...
              {loading ? <div className="skeleton" style={{ height:200 }} /> :
                expensePieData.length ? (<>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={expensePieData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value">
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 12px", marginTop:8 }}>
                    {expensePieData.map((e,i) => (
                      <span key={i} style={{ fontSize:11, display:"flex", alignItems:"center", gap:4 }}>
                        <span style={{ width:8, height:8, borderRadius:"50%", background:COLORS[i%COLORS.length], display:"inline-block" }} />{e.name}
                      </span>
                    ))}
                  </div>
                </>) : <div style={{ color:"#94a3b8", fontSize:13, paddingTop:40, textAlign:"center" }}>No expense data</div>
              }
            </div>

            {/* Payment mode pie */}
            <div className="chart-card">
              <SectionHeader title="Payment Modes" sub="Bills by payment method" />
              {loading ? <div className="skeleton" style={{ height:200 }} /> :
                paymentPieData.length ? (<>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={paymentPieData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value">
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 12px", marginTop:8 }}>
                    {paymentPieData.map((p,i) => (
                      <span key={i} style={{ fontSize:11, display:"flex", alignItems:"center", gap:4 }}>
                        <span style={{ width:8, height:8, borderRadius:"50%", background:COLORS[i%COLORS.length], display:"inline-block" }} />{p.name} ({p.value})
                      </span>
                    ))}
                  </div>
                </>) : <div style={{ color:"#94a3b8", fontSize:13, paddingTop:40, textAlign:"center" }}>No payment data</div>
              }
            </div>

            {/* Size performance */}
            <div className="chart-card">
              <SectionHeader title="Size Performance" sub="Stock quantity by size" />
              {loading ? <div className="skeleton" style={{ height:200 }} /> :
                sizePerf.length ? (
                    ...
                    {sizePerf.map((s,i) => (
                      <div key={s.size}>
                        ...
                        <div style={{ background:"#f1f5f9", borderRadius:99, height:7 }}>
                          <div style={{ height:7, borderRadius:99, width:`${(s.qty/sizePerf[0].qty)*100}%`, background:COLORS[i%COLORS.length], transition:"width .4s ease" }} />
                        </div>
                      </div>
                    ))}
                  </div>
                ) : <div style={{ color:"#94a3b8", fontSize:13, paddingTop:40, textAlign:"center" }}>No variant data</div>
              }
            </div>
          </div>
          ...
          {/* P&L Overview bar */}
          <div className="chart-card" style={{ marginBottom:24 }}>
            <SectionHeader title="P&L Overview" sub="Revenue vs Cost vs Profit at a glance" />
            {loading ? <div className="skeleton" style={{ height:200 }} /> : (
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  <Bar dataKey="value" name="Amount" radius={[6,6,0,0]}>
                    {["#6366f1","#f59e0b","#a78bfa","#f43f5e",netProfit>=0?"#10b981":"#ef4444"].map((fill,i) => <Cell key={i} fill={fill} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            )}
          ...
          {/* Top Vendors */}
          <div className="chart-card" style={{ marginBottom:24 }}>
            <SectionHeader title="Top Vendor Performance" sub="Ranked by total purchase amount" />
            {loading ? <div className="skeleton" style={{ height:180 }} /> :
              topVendors.length ? (
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24 }}>
                  <table className="pl-table">
                    <thead><tr><th>#</th><th>Vendor</th><th style={{ textAlign:"right" }}>Orders</th><th style={{ textAlign:"right" }}>Total</th></tr></thead>
                    <tbody>
                      {topVendors.map((v,i) => (
                        <tr key={i}>
                          <td style={{ color:"#94a3b8", width:28 }}>{i+1}</td>
                          <td style={{ fontWeight:600 }}>{v.vendorName}</td>
                          <td style={{ textAlign:"right" }}>{v.purchaseCount}</td>
                          <td style={{ textAlign:"right", fontWeight:700 }}>{fmtShort(v.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <ResponsiveContainer width="100%" height={Math.max(topVendors.length*36, 160)}>
                    <BarChart data={topVendors} layout="vertical" margin={{ top:0, right:16, left:0, bottom:0 }}>
                      <XAxis type="number" tick={{ fontSize:10, fill:"#94a3b8" }} tickFormatter={fmtShort} />
                      <YAxis dataKey="vendorName" type="category" tick={{ fontSize:10, fill:"#374151" }} width={90} />
                      <Tooltip content={<ChartTip />} />
                      <Bar dataKey="total" name="Total" fill="#6366f1" radius={[0,4,4,0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ) : <div style={{ color:"#94a3b8", fontSize:13, textAlign:"center", padding:"24px 0" }}>No vendor data for this period</div>
            }
          </div>
          ...
          {topSellingItems.length > 0 && (
            <div className="chart-card" style={{ marginBottom:24 }}>
              <SectionHeader title="Top Selling Items" sub="By quantity sold in billing" />
              <table className="pl-table">
                <thead><tr><th>#</th><th>Item</th><th>Code</th><th style={{ textAlign:"right" }}>Qty Sold</th></tr></thead>
                <tbody>
                  {topSellingItems.map((t,i) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign:"right", fontWeight:700 }}>{t.totalSold}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Expense Details */}
          <div className="chart-card" style={{ marginBottom:24 }}>
            <SectionHeader title="Expense Details" sub="All expenses in selected period" />
                  ...
                  <table className="pl-table">
                    ...
                    <tbody>
                      {expenses.slice(0,20).map((e,i) => (
                        <tr key={i}>
                          <td style={{ color:"#64748b", fontSize:12 }}>{e.expense_date??e.date}</td>
                          <td><span className="tag tag-blue">{e.category_name??e.category??"–"}</span></td>
                          <td style={{ color:"#64748b" }}>{e.description??"–"}</td>
                          <td style={{ textAlign:"right", fontWeight:700, color:"#f43f5e" }}>{fmt(e.amount)}</td>
                        </tr>
                      ))}
                      <tr className="total-row">
                        <td colSpan={3} style={{ fontWeight:800 }}>Total Expenses</td>
                        <td style={{ textAlign:"right", fontWeight:800, color:"#f43f5e" }}>{fmt(totalExpenses)}</td>
                      </tr>
                    </tbody>
                  </table>
                  {expenses.length > 20 && <p style={{ fontSize:12, color:"#94a3b8", textAlign:"center", marginTop:8 }}>Showing 20 of {expenses.length} entries</p>}
                </div>
              ) : <div style={{ color:"#94a3b8", fontSize:13, textAlign:"center", padding:"24px 0" }}>No expenses for this period</div>
            }
          </div>

          <div style={{ textAlign:"center", color:"#cbd5e1", fontSize:11, paddingTop:12 }}>
            Generated on {new Date().toLocaleString("en-IN")} · {startDate} to {endDate}
          </div>
