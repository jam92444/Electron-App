...
function registerVendorHandlers(db) {
  ...
  ipcMain.handle("db:insertVendor", (e, vendor) => {
    try {
      ...
      const exists = db
        .prepare("SELECT 1 FROM vendors WHERE email = ? OR phone = ?")
        .get(vendor.email, vendor.phone);

      if (exists) {
        return { success: false, error: "VENDOR_ALREADY_EXISTS" };
      }
  ...
  ipcMain.handle("db:updateVendor", (e, vendor) => {
    try {
      const exists = db.prepare("SELECT 1 FROM vendors WHERE id = ?").get(vendor.id);
